...
/**
 * We have a small web server that receive requests with the method
 * getRequest();
 * The first parameter is the path of the request, and the second is an object
 * that represents the
 ...
 * The web server writes directly its response to the console using
 * System.out.println.
 *
 * When a non authorized user wants to access the content, the webserver must
 * respond with the message :
 * "Status 403 : user is not authorized to access this content"
 *
 * When we try to access content that do not exist, the server must respons with
 * the message :
 ...
 * In order to log all request made to the server in a log file, we are going to
 * use the
 * Observer Pattern
 ...
 * request made to the web server should notify all the registered observers with the
 * WebRequest object.
 * 2.b - Modify the FileLogger to be an implementation of WebRequestObserver, in
 * order for it to log each time it is notified of a request
 * 2.c - Tie all of this together in your main
 *
 *
 * 3 - Bonus. If you want, you can undo the Observer pattern, and try to solve
 * the same problem (logging requests in a file) by using the Proxy pattern
 */
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        RequestHandler existingContentCheckRequestHandler = new ExistingContentCheckRequestHandler();
        RequestHandler policyCheckRequestHandler = new PolicyCheckRequestHandler();
        RequestHandler renderContentHandler = new RenderContentHandler();

        existingContentCheckRequestHandler.setSuccessor(policyCheckRequestHandler);
        policyCheckRequestHandler.setSuccessor(renderContentHandler);

        WebServer webServer = new WebServer(existingContentCheckRequestHandler);
        //2.c - Tie all of this together in your main
        FileLogger fileLogger = new FileLogger("logs.txt");
        WebServiceProxy webServiceProxy = new WebServiceProxy("logs2.txt", webServer);
         ...
         * Status 403 : user is not authorized to access this content
         */
        webServiceProxy.getRequest(new WebRequest("/dashboard", regularUser));

        /**
         * Expected output :
         * Status 404 : Page missing
         */
        webServiceProxy.getRequest(new WebRequest("/dashboard/nonExistingPage", adminUser));

        /**
         * Expected output :
         * Status 200 : Dashboard content here
         */
        webServiceProxy.getRequest(new WebRequest("/dashboard", adminUser));

        /**
         * Expected output :
         * Status 200 : Home content here
         */
        webServiceProxy.getRequest(new WebRequest("/home", regularUser));
